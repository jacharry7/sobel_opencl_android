...
#define LIB_GLES_MALI1 "/system/vendor/lib/egl/libGLESv1_CM_adreno.so"
#define LIB_GLES_MALI2 "/system/vendor/lib/egl/libGLESv2_adreno.so"
#define LIB_LLVM   "/system/vendor/lib/libllvm-qcom.so"
...
static void *getCLHandle(){
  ...
  if(res != NULL){
    ...
    res = dlopen(LIB_GLES_MALI1,RTLD_LAZY);
  }
  if(res != NULL){
    so_name = LIB_GLES_MALI1;
  }else{
    res = dlopen(LIB_GLES_MALI2,RTLD_LAZY);
  }
   if(res != NULL){
    so_name = LIB_GLES_MALI2;
  }else{
